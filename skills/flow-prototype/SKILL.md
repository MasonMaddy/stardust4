...
```
Orient → Scope → Scaffold → Build flow → Verify loop → Device + motion → Capture → Publish
```
...
Before any code, read everything the handoff ships — the **PRD/README** (intent, copy, states),
...
**directions**, and how much of the **state matrix**. One `AskUserQuestion` here prevents building
the wrong surface across every direction. Then build slice-by-slice (see the verify loop).
