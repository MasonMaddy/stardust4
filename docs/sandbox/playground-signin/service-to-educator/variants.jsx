 ...
 *
 * "Service to Educator" fork: the service-login step is replaced with the Immersive teal
 * direction (full-bleed, left-aligned, translucent fields — see variants.jsx Service6) rendered
 * INSIDE the same persistent hero element at height:'100%' (the sheet is squeezed to nothing
 * behind it). On sign-in the hero shrinks to the normal educators height (250), so the same
 * height transition now reads as the white educator-list sheet sliding up from the bottom.
 * ==================================================================== */
const FLOW3_HERO = {
...
function Flow3({ step, ctx, onSignIn }) {
  ...
  const hero = isService ? { h: '100%' } : (FLOW3_HERO[step] || FLOW3_HERO.educators);
  ...
  const headTitle = !isService && hero.head ? hero.head[0] : (cfg ? cfg.title : '');
  const headSub = !isService && hero.head ? hero.head[1] : (cfg ? cfg.subtitle : '');
  const navKind = isService ? null : (cfg && cfg.nav);
  const onNav = isService ? null : (cfg && cfg.onNav);

  let body = null, footer = null, footerAuto = false;
  if (!isService) {
    body = cfg.children;
    footer = cfg.footer;
  }
  // (service fields live inside the immersive hero itself, below — the sheet stays empty
  // until the hero shrinks and reveals it)
  ...
  const heroEl = isService ? (
    <div className="v3-hero" style={{ height: hero.h, background: V_IMMERSIVE, position: 'relative', flexShrink: 0, boxSizing: 'border-box' }}>
      <VScene h={300} />
    ...
    <div className="v3-hero" style={{ height: hero.h, background: V_HERO_GRAD, position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 28px', boxSizing: 'border-box', flexShrink: 0 }}>
      ...
      <div key={'h-' + step} className="v3-herocontent" style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
        <HeroBrand step={step} educator={ed} size={hero.logo} float={false} />
