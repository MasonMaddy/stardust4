...
| `version-0.4/` | **The prototype.** Full S1–S7 flow, phone ⇄ tablet, plus `standalone.html` (un-shelled web build) and the anatomy inspector (`inspect-goggles.js`). Includes the immersive service login + sheet reveal and the flow-wide travelling-mark transition. | Current |
| `service-to-educator/` | Fork that pioneered the immersive teal login + bottom-sheet educator reveal. | Adopted into v0.4 — kept as the exploration record |
| `motion-lab/` | Direction D "travelling logo": Service ⇄ Educator transition study (fade-through + one shared brand-mark element) with SwiftUI/Compose notes. | Adopted into v0.4 — kept as the exploration record |
...
v0.3 → **v0.4 (current)** — folds in the service-to-educator sheet reveal and the
motion-lab travelling-logo transition.
